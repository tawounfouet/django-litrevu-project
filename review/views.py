...
def get_users_viewable_reviews(request):
    ...
    # Union des critiques des utilisateurs suivis et des critiques liées aux tickets
    reviews_users_followed = reviews_users_followed | Review.objects.filter(
        ticket__user_id=request.user.id
    )
...
def feed(request):
    ...
    # Récupérez les tickets visibles par l'utilisateur
    # avec le nombre de critiques associées
    ...
    tickets = tickets.annotate(
        content_type=Value("TICKET", CharField()), review_number=Count("review")
    )
...
class TicketCreateView(CreateView):
    ...
    template_name = (
        "review/ticket_create.html"  # Remplacez par le nom de votre template
    )
...
def delete_ticket(request, ticket_id):
    ...
    if request.user != ticket.user:
        # Si non, rediriger l'utilisateur vers la liste des tickets
    ...
    return render(
        request, "review/ticket_delete.html", {"form": form, "ticket": ticket}
    )
...
def new_review_blank(request):
    """creer une critique avec deux formulaires :
    le formulaire de ticket et le formulaire de review en utilisant
    une view basée sur une fonction"""
    user = request.user
    ...
    return render(
        request,
        "review/review_create_blank.html",
            ...
            if username_to_follow == request.user.username:
                ...
                try:
                    ...
                    if is_following:
                        ...
                        follow.save()
                        messages.success(
                            request, f"Vous suivez maintenant {username_to_follow} !"
                        )
                except User.DoesNotExist:
                    messages.error(
                        request, f"Utilisateur {username_to_follow} n'existe pas."
                    )
...
def unfollow_user(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Vous vous êtes désabonné de {user_to_unfollow.username}."
            )
            return redirect(
                "review:subscriptions"
            )  # Redirigez vers la page d'accueil ou une autre vue.
    else:
